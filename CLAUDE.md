...
- **Fase 3 (catálogo) ✅ (v1)**:
  ...
  - Catálogo con filtro por categoría (`/productos`) + ficha de detalle (`/productos/[slug]`, prerenderizada por SEO).
  - **Ficha de producto = configurador reducido ✅ (v2)**: como el diseño ya está hecho, no hay texto ni tipografía; se elige **color (incluido RGB, con swatch conic-gradient)**, **tamaño**, **contorno** (los mismos `NEON_SUPPORTS`) y un **interruptor "Resistente al agua (IP65)"** (el uso `exterior`). La página pasa `options` de `getConfiguratorOptions()`, así que soportes, usos y suplemento RGB salen de la BD igual que en `/personalizar`.
    - Precio con **`calcProductPrice(basePrice, opts, options)`** en `lib/pricing.ts` (función PURA, cliente + servidor). A diferencia del personalizado **no** parte de la geometría: el fabricante ya tiene tarifado el diseño, así que es `precio de catálogo + tamaño + soporte + RGB, × uso`. El delta por tamaño vive en `PRODUCT_SIZES` (`lib/products.ts`), no en la BD.
    - `priceCart` recalcula igual en servidor: el precio base sale SIEMPRE de la BD y las opciones se sanean contra las válidas (una opción inventada cae al valor por defecto). Verificado: XL+RGB+corte a la letra+exterior = 609 €, opciones falsas = 249 €, y los carritos antiguos (solo hex + talla) siguen valorándose bien.
    - Los ítems de carrito de tipo `product` guardan `colorId/supportId/usageId` + `breakdown`; son **opcionales** porque los carritos guardados antes de la v2 no los traen (se cae a `DEFAULT_PRODUCT_OPTIONS`).
    - ⚠️ **`addToCart` sella `addedAt`**: los llamantes pasan el ítem sin marca de tiempo (tipo `NewCartItem`). Se hizo así porque `react-hooks/purity` rechaza `Date.now()` dentro del cuerpo de un componente.
...
- **Panel de administración ✅ (v2, endurecido)** en `/admin`:
  ...
  - Secciones: **Resumen**, **Pedidos**, **Solicitudes**, **Productos**, **Precios**, **Opiniones** (Trustpilot), **Seguridad** (sesiones propias; todas si superadmin + botón pánico), **Usuarios** (solo superadmin) y **Manual** (botón `?` de la barra).
  - **Manual de uso ✅** en `/admin/manual`: guía completa para quien lleva la tienda (pedidos, EPS, solicitudes, alta/baja de productos, cómo preparar un SVG, precios, Trustpilot, usuarios, y una sección explícita de limitaciones actuales). Va como página y no como PDF para que no circulen copias desfasadas; lleva `PRINT_CSS` para que `Ctrl+P → Guardar como PDF` dé un documento legible. **Si se añade o cambia una funcionalidad del panel, hay que actualizarlo.**
  - **`/admin/precios` incluye el bloque Envío** (grupo `SHIPPING`: coste y umbral de gratuidad). ⚠️ `updatePricingRuleAction` revalida `/`, `/personalizar` y `/checkout` además de la propia pantalla: la portada monta el configurador **y** anuncia el "envío gratis desde X", y el checkout calcula los portes.
  - **Alta y baja de productos ✅**:
    ...
    - **`components/shop/product-artwork.tsx`**: único sitio donde se decide cómo se dibuja un producto (SVG / emoji / texto). Lo usan `ProductCard`, `ProductDetail`, el **carrito** y el admin. `/productos`, `/productos/[slug]` y `/carrito` cargan `configuratorFontVars` porque un producto puede llevar tipografía del configurador.
      - ⚠️ El carrito solo guarda **nombre y color** en `localStorage`, así que no puede dibujar el diseño por sí mismo. `/carrito` (server component) trae el mapa `slug → ProductArtworkData` con `getProductArtworks()` y se lo pasa a `CartView`. Ventaja añadida: si el admin cambia el diseño, el carrito no se queda con el viejo. Se manda el catálogo entero porque el servidor no sabe qué hay en el carrito — hoy son ~5 KB; con cientos de productos vectoriales habría que pasar a un endpoint que reciba los slugs.
...
- **EPS de producción ✅** (lo que pide el fabricante: "EPS a tamaño real trazado"):
  - Botón **Descargar EPS** en cada línea de `/admin/pedidos` — **las tres formas de diseño**: personalizado del configurador, producto de catálogo **vectorial** y producto de catálogo **con tipografía**. Ruta `app/admin/pedidos/[itemId]/eps/route.ts` protegida con `isAdmin()` (verificado: anónimo→login, cookie falsa/caducada→401). Si el producto se borró del catálogo, devuelve 404 explicando que ya no hay diseño que exportar.
  - **Diseños vectoriales** → `buildVectorEps()` + **`lib/svg-path.ts`**: convierte el SVG saneado en comandos absolutos `M/L/C/Z` con las transformaciones ya aplicadas. Resuelve arcos elípticos (anexo F.6 de SVG → cúbicas), cuadráticas, atajos `H/V/S/T` y las primitivas (`rect` con esquinas redondeadas, `circle`, `ellipse`, `line`, `polyline`, `polygon`). ⚠️ El lector de `d` es **carácter a carácter**, no un tokenizador de números: los *flags* del arco pueden venir pegados en SVG minificados (`a5 5 0 0110 0`) y un tokenizador leería `0110` como un solo valor. Verificado contra los 8 casos de sintaxis y comparando el dibujo reconstruido con el original.
  - **Regla de tamaño de un logo**: no tiene "altura de letra", así que el diseño se inscribe en el lado mayor del tamaño contratado (Gigante = hasta 150 cm → el lado mayor mide 150 cm). Queda anotado en la cabecera del EPS.
